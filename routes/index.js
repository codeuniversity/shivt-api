  ...
  if (token) {
    ...
    jwt.verify(token, cert, {algorithm: ['RS256']}, (err, decod) => {
      if (err) {
        ...
        if(!isNaN(decod)) {
          if(
            ((RegExp('/v1/employees/(.*)/shifts').test(req.url))
            && req.method === 'GET') || ((RegExp('/v1/employees/(.*)').test(req.url))
            && req.method === 'GET') || ((RegExp('/v1/events/(.*)/shifts/(.*)').test(req.url))
              && req.method === 'GET') ||
              RegExp('/v1/employees/(.*)/blocked').test(req.url) ||
              RegExp('/v1/employees/(.*)/blocked/(.*)').test(req.url)
              ){
                console.log('pass')
                req.decoded = decod
    ...
    if(!RegExp('/v1/employees/login').test(req.url)) {
